...
export default function LearningPanel({ node }: LearningPanelProps) {
  ...
  const { closeLearningPanel, showToastMessage } = useUIStore()
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      {/* Panel */}
      <div className="relative ml-auto w-full max-w-4xl bg-slate-900 border-l border-slate-700 overflow-y-auto flex">
        {/* Left: Knowledge */}
        <div className="flex-1 p-6 border-r border-slate-700 overflow-y-auto">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              onClick={closeLearningPanel}
              className="text-slate-400 hover:text-slate-200 text-xl p-2"
            >
              ✕
            </button>
          </div>
          ...
          {node.knowledge.complexity && (
            <div className="flex gap-4 mb-6">
              <div className="px-3 py-2 bg-slate-800 rounded-lg">
                <div className="text-[10px] text-slate-500 uppercase">时间</div>
                <div className="text-sm text-blue-400 font-mono">
                  {node.knowledge.complexity.time}
                </div>
              </div>
              <div className="px-3 py-2 bg-slate-800 rounded-lg">
                <div className="text-[10px] text-slate-500 uppercase">空间</div>
          ...
          {node.knowledge.template && (
            <div className="mb-6">
              <button
                ...
                className="text-sm font-semibold text-slate-300 mb-3 flex items-center gap-2 hover:text-blue-400 transition-colors"
              >
          ...
          {learningState === 'completed' && (
            <div className="text-center py-4">
              ...
              <div className="text-green-400 font-semibold">节点已完成！</div>
            </div>
          )}
        </div>
                ...
                >
                  ...
                  {problem.template && (
                    <details className="mt-2">
                      ...
                      <pre className="mt-2 p-2 bg-slate-950 rounded text-[11px] text-slate-400 overflow-x-auto font-mono">
                        {problem.template}
                      </pre>
                    </details>
                  )}
